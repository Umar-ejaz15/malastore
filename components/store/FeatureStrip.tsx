...
export function FeatureStrip() {
  return (
    <div className="bg-beige border-y border-grey-light/60 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6">
          {features.map((f, i) => (
            <div
              key={f.label}
              className={`flex items-start gap-3 px-4 sm:px-6 ${i < features.length - 1 ? 'md:border-r border-grey-light/60' : ''}`}
            >
              <div className="text-gold shrink-0 mt-0.5">{f.icon}</div>
              <div>
                <p className="font-ui font-semibold text-navy text-[11px] tracking-widest uppercase">{f.label}</p>
                <p className="font-body text-grey text-xs mt-0.5">{f.sub}</p>
              </div>
            </div>
          ))}
        </div>
