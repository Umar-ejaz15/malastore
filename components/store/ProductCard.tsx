    ...
    <Link href={`/products/${product.slug}`} className="group block cursor-pointer">
      ...
      >
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10">
          {product.isNew && (
            <span className="inline-block bg-navy text-white font-ui font-bold px-2.5 py-1 rounded-full text-[9px] tracking-widest uppercase shadow-sm">
              New In
            </span>
          )}
          {product.isRestocked && (
            <span className="inline-block bg-emerald text-white font-ui font-bold px-2.5 py-1 rounded-full text-[9px] tracking-widest uppercase shadow-sm">
              Restocked
            </span>
          )}
          {discount && (
            <span className="inline-block bg-coral text-white font-ui font-bold px-2.5 py-1 rounded-full text-[9px] tracking-widest uppercase shadow-sm">
              -{discount}%
            </span>
          )}
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out z-10">
          <button
            ...
            className={`w-full backdrop-blur-sm text-white font-ui font-semibold py-3.5 transition-all duration-200 text-[11px] uppercase tracking-widest flex items-center justify-center gap-2 ${
              adding
                ? 'bg-emerald'
                : 'bg-navy/95 hover:bg-navy'
            }`}
          >
            {adding ? (
              <>
      ...
      <div className="pt-4 pb-1">
        <p className="font-ui text-[10px] text-grey uppercase tracking-widest mb-1.5">
          {product.category}
          {product.pieces && (
            <span className="ml-2 text-grey/60">· {product.pieces}pc</span>
          )}
        </p>
        <h3 className="font-display font-semibold text-navy leading-snug mb-2.5 text-[15px] group-hover:text-gold transition-colors duration-300 line-clamp-2">
          {product.name}
        </h3>
        ...
        <div className="flex items-baseline gap-2">
          ...
          {product.originalPrice && (
            <span className="font-body text-grey/60 line-through text-sm">
        ...
        <div className="flex flex-wrap gap-1 mt-2.5">
          {product.sizes.slice(0, 5).map((s) => (
            <span
              key={s}
              className="font-ui text-[9px] uppercase tracking-wider text-grey/70 border border-grey-light px-1.5 py-0.5 rounded"
            >
              {s}
            </span>
          ))}
          {product.sizes.length > 5 && (
            <span className="font-ui text-[9px] text-grey/50">
              +{product.sizes.length - 5}
            </span>
          )}
        </div>
